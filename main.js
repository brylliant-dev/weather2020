  ...
  map.on("style.load", () => {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        clickedLat = latitude;
        $("#calendar-title").text("15-days Forecast (High)");
        // Show preloader
...
// Get the current UTC date
  ...
  let OverlapValue = 1;
  // Add 15 days to the current date and check for overlap
  currentDate.setUTCDate(currentDate.getUTCDate() + 15);
  // Check if the next month overlaps with the current month
  const nextMonth = getNextMonth(currentDate);
  if (nextMonth.getUTCMonth() === currentMonth) {
    OverlapValue = 2;
  } else {
    OverlapValue = 1;
  }

  for (let i = 0; i < OverlapValue; i++) {
  ...
  const toggleTemperatureUnit = () => {
    ...
    if (currentDataType === "tmax") {
      ...
      $("#calendar-title").text("15-days Forecast (Low)");
      fetchData(
        clickedLat,
        clickedLong,
        "tmin",
      ...
      $("#calendar-title").text("15-days Forecast (Snow)");
      fetchData(
        clickedLat,
        clickedLong,
        "snow",
      ...
      $("#calendar-title").text("15-days Forecast (PRCP)");
      fetchData(
        clickedLat,
        clickedLong,
        "prcp",
