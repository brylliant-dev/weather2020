//revert old script 3month ahead
...
$(document).ready(function () {
  ...
  const formatDate = (date) => {
    ...
    const month = (date.getUTCMonth() + 1).toString().padStart(2,
      "0"); // Use getUTCMonth() for UTC month
  ...
  const getNormalsData = async (lat, lon, month, year) => {
    ...
    const apiUrl =
      `https://api.weather2020.com/normals?lat=${lat}&lon=${lon}&units=${units}&month=${month}&year=${year}`;
