...
for (let i = 0; i < 6; i++) {
    ...
    const nextMonth = nextMonthDate.getUTCMonth() + 1; // Months are 0-indexed, so add 1

    currentDate = nextMonthDate;
        // Get the month name in 3 letters
        var monthName = new Date(
            Date.UTC(currentDate.getUTCFullYear(), nextMonth - 1, 1 + 1) // Use Date.UTC() for UTC-based Date object
                                ...
                                const determineBackgroundColor = (normalTmax) => {
                                  ...
                                  if (units === "metric") {
                                    ...
                                    if (isSnow) {
                                      ...
                                      return item[dataType] <= prcpResult
                                      ? "transparent"
                                      : "prcp-background";
                                    }
                                  } else {
